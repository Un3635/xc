<template>
  <div class="order">
    ...
    <div class="content">
          ...
          <td colspan="7">
            <div class="order-summary-info">
              <dl class="order-summary__list">
                ...
                <div class="order-summary__item" v-if="order.discountAmount>0">
                  <dt>已优惠金额:</dt>
                  <dd>{{order.discountAmount|currency}}元</dd>
                </div>
              </dl>
              <a class="right link" target="_blank" :href="order._presellUrl"
                 v-if="selected.status === 1 && order.orderInfoDetails[0].goodsType === 0 || selected.status === 3 && order.orderInfoDetails[0].goodsType === 0">
                下载预售合同
              </a>
              <!-- <a class="right link" target="_blank" :href="order._presellUrl"
                ...
                <tr v-for="(goodsItem,$i) in order.orderInfoDetails" :key="goodsItem.batchGoodsId">
                  <td>
                    <div class="goods-detail">
                      ...
                      <p :title="goodsItem.goodsName">
                        <span v-if="goodsItem.goodsType===1">合约</span>
                        {{goodsItem.goodsName}}
                      </p>
                      <p class="text-grey" :title="goodsItem.goodsSubTitle">{{goodsItem.goodsSubTitle}}</p>
                    </div>
                  </td>
                  <td v-if="goodsItem.goodsType===0">{{goodsItem.startSendTime|date('YYYY-MM-DD')}}&nbsp;至&nbsp;{{goodsItem.endSendTime|date('YYYY-MM-DD')}}</td>
                  <td v-else-if="goodsItem.goodsType===1">--</td>
      ...
      <el-dialog title="取消订单" :visible.sync="dialogVisible" width="420px" custom-class="custom">
        <span>是否要取消订单吗？</span>
        <span slot="footer">
      ...
      <el-dialog title="坐享收益，一键托管" :visible.sync="hostingDialogVisible" width="420px" custom-class="custom">
        ...
        <dl class="hosting-info">
          <div class="hosting-info__item">
            <dt>新增托管</dt>
            <dd>
              <el-input-number :min="1" :max="hostingInfo.maxHostingCount" v-model="hostingInfo.num"/>
            </dd>
          </div>
          <div class="hosting-info__item">
            <dt>
      ...
      <el-dialog :visible.sync="expressDialogVisible" width="420px" custom-class="custom">
