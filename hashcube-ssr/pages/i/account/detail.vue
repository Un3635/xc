  ...
  <div class="account-detail" @click="closeDetailPopover">
    ...
    <div class="header-date-change">
      ...
      <el-select class="xc-select no-radius" popper-class="select-recent " size="medium" @change="selectChange"
                 v-model="flag" placeholder="全部" @focus="hidePopover()">
        <el-option
    ...
    <table class="table" id="tbContainer">
      ...
      <tr v-if="tableData.length > 0" v-for="(item,index) in tableData" :key="index">
        ...
        <td class="text-center" @click.stop>
          <span class="btn-look" @click="showDetail(item,$event)"
                v-if="item.type===1&&item.typeOfOperation!==0&&item.typeOfOperation!==3">
            <span>查看</span>
  ...
  export default {
    ...
    mounted() {
      ...
      AccountApi.userAccountCurrencyList()
        ...
        .then(res => res.result || [])
        .then(res => {
          let item = res.find(item => item.currencyId === currencyId);
