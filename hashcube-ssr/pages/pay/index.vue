        ...
        <ul class="pay-methods-list">
          ...
          <li :class="itemClassList(PaymentMethods.QUICK_PAY)"
            ...
            <span v-if="userInfo.isBindBankCard">
              ...
              <span
                class="txt">单笔限额{{bankCardInfo.singleLimit | currency}}元&nbsp;|&nbsp;每日限额{{bankCardInfo.dailyLimit | currency}}元</span>
            </span>
            <span v-else>
          ...
          <div class="pay-offline__container">
            <ul>
              ...
              <li class="step">
                ...
                <div class="step-content">
                  提交汇款信息后，您可以在
                  <nuxt-link to="i/order">个人中心-我的订单</nuxt-link>
...
<script>
  ...
  import {getBankInfo, removeStore} from '../../utils';
  ...
  export default {
    ...
    computed: {
      ...mapState(['token', 'userInfo', 'trackerGoodsItem']),
      payNextDisabled() {
        ...
        this.$confirm('此操作将扣除您的账户余额，是否继续?', '提示', {
          ...
          .then(res => {
            ...
            if (this.goodsType !== 1) {
              ...
              this.$router.replace('/i/order?selected=2');
            }
          })
          .catch((err) => {
            if (err === 'cancel') {
...
//              this.$router.replace({path: '/i/order'});
    ...
    mounted() {
      ...
      if (localStorage[TRACKER_GOODS_ITEM_KEY]) {
        this.$store.commit(UPDATE_TRACKER_GOODS_ITEM, JSON.parse(localStorage[TRACKER_GOODS_ITEM_KEY]));
      }
    },
    destroy() {
      if (this.timer) {
