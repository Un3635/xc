  ...
  <div class="page">
    <div class="page-container">
      <div class="text-right">
        <nuxt-link class="link" to="/sign-in">已有账号，立即登录 &gt;</nuxt-link>
      </div>
      <div class="login-container">
        <div class="figure">
          <img src="/images/login-logo.png" class="figure-logo">
        </div>
        <div class="x-tabs">
          <div class="x-tabs__header">
            <ul class="x-tabs__nav">
              <li class="x-tabs__item" :class="{active: isPhoneRegister}" @click="isPhoneRegister=true">手机注册</li>
              <li class="x-tabs__item" :class="{active: !isPhoneRegister}" @click="isPhoneRegister=false">邮箱注册</li>
            </ul>
          </div>
          <div class="x-tabs__content">
            <div class="tabs-pane" v-if="isPhoneRegister">
              <!-- 手机注册 -->
              <form class="login-form">
                ...
                <div class="input-group">
                  <x-captcha v-model="phoneForm.captcha" key="phoneForm.captcha" @ready="captchaInstance=$event"/>
                </div>
                <div class="input-group">
                  <el-input class="code-input" placeholder="请输入验证码" name="code"
                    ...
                    <el-button slot="suffix" class="text-secondary" type="text" @click="sendPhoneRegisterCode">
                      {{phoneBtnText}}
                    </el-button>
                  </el-input>
                ...
                <div class="input-group">
                  <el-input placeholder="邀请者邀请码（选填）" v-model="phoneForm.inviteCode"></el-input>
                </div>
                <el-button type="primary" class="btn-login" @click="phoneRegister" :disabled="isPhoneRegDisabled">
                  注册
                </el-button>
              </form>
            </div>
            <div class="tabs-pane" v-if="!isPhoneRegister">
              ...
              <form class="login-form">
                ...
                <div class="input-group">
                  <x-captcha v-model="emailForm.captcha" :key="'emailForm.captcha'" @ready="captchaInstance=$event"/>
                </div>
                <div class="input-group">
                  <el-input class="code-input" placeholder="请输入验证码" name="code"
                            v-model="emailForm.code" data-vv-scope="$email" v-validate="'required|code'"
                            @focus="errors.remove('code','$email')" key="$phone.code">
                    <el-button slot="suffix" class="text-secondary" type="text" @click="sendEmailRegCode">
                      {{emailBtnText}}
                    </el-button>
                  </el-input>
                ...
                <div class="input-group">
                  <el-input placeholder="邀请者邀请码（选填）" v-model="emailForm.inviteCode"></el-input>
                </div>
                <el-button type="primary" class="btn-login" @click="emailRegister" :disabled="isEmailRegDisabled">
                  注册
                </el-button>
              </form>
            </div>
...
<script>
  ...
  export default {
    ...
    computed: {
      isPhoneRegDisabled() {
        return !this.phoneForm.phone || !this.phoneForm.code || !this.phoneForm.password || !this.phoneForm.confirmPwd;
      },
      isEmailRegDisabled() {
        return !this.emailForm.email || !this.emailForm.code || !this.emailForm.password || !this.emailForm.confirmPwd;
      }
    },
    methods: {
      phoneRegister() {
        if (this.isPhoneRegDisabled) {
          return;
        }
        const {captcha, phone, password, code, codeToken, inviteCode} = this.phoneForm;
        const firChannelId = this.$route.query.firChannelId || sessionStorage.firChannelId || '';
        const secChannelId = this.$route.query.secChannelId || sessionStorage.secChannelId || '';
        this.$validator.validateAll('$phone')
          ...
          .then(() => {
            ...
            return SignAPI.register(params);
          })
      ...
      sendPhoneRegisterCode() {
        ...
        this.$validator.validate('$phone.phone', phone, '$phone')
          .then(toPromise)
          .then(() => {
            if (captcha) {
              return captcha;
          ...
          .then(res => {
            this.phoneForm.codeToken = res.result;
      ...
      sendEmailRegCode() {
        ...
        this.$validator.validate('$email.email', email, '$email')
          .then(toPromise)
          .then(() => {
            if (captcha) {
              return captcha;
          ...
          .then(res => {
            this.emailForm.codeToken = res.result;
            this.$message.success('验证码发送成功');
      ...
      emailRegister() {
        if (this.isEmailRegDisabled) {
          return;
        }
        const {email, password, code, inviteCode, codeToken} = this.emailForm;
        const firChannelId = this.$route.query.firChannelId || sessionStorage.firChannelId || '';
        const secChannelId = this.$route.query.secChannelId || sessionStorage.secChannelId || '';
...
<style scoped lang="scss">
  .page {
    ...
    bottom: 0;
    right: 0;
    background: url("/images/login-bg.png") no-repeat 0 -5px;
