...
/**
 * 根据总页数和当前页获取页面上展示的分页页码信息,展示10个
 * @param {number} totalPages
 * @param {number} currentPage
 * @returns {Array<number>}
 */
export function getPageList(totalPages, currentPage = 1) {
  // 页码列表
  let rList = range(1, totalPages + 1);
  // 获取展示的起始页码
  let startPn = currentPage - 5 > 0 ? currentPage - 5 : 1;
  // 获取起始页码在页码列表中的索引
  let sIndex = rList.indexOf(startPn);
  // 从起始页码位置开始取10元素
  return rList.slice(sIndex, sIndex + 10);
}
