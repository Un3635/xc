  ...
  <div class="company_certification">
    ...
    <div class="form_box">
      <div class="one_line">
        ...
        <input type="text" placeholder="姓名" @input="account_name" v-model.trim="account_user_name" class="ipt"
               @focus="iptFocus">
        <span class="account_tip">{{account_tip}}</span>
      </div>
      <div class="one_line">
        <span class="name">公司名称</span>
        <input type="text" placeholder="公司名称" @input="company_na" v-model.trim="company_name" class="ipt_two"
               @focus="iptFocusT">
      ...
      isFromCF(){
        ...
        vm.$http.get('/account/queryAuthInfo', {params: {token: vm.token,initTime: new Date().getTime()}}).then(function (data) {
          ...
          if (Number(res.code) === 1000) {
            ...
            vm.attachmentName = res.data.attachmentName;
            vm.fileList = [{name: '', url: ''}];
            vm.fileList[0].url = vm.licenseImg;

            vm.pic[0] = vm.attachmentName;
