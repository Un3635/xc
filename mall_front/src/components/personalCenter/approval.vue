<template>
  <div class="approval">
    <section class="approval-info">
      <header>
        ...
        <div class="approver" v-if="approvalOrderInfo.recordProcessList">
          <h3>审批人</h3>
          <ul class="clearfix">
            <li class="fl" v-for="item,index in approvalOrderInfo.recordProcessList" :key="index">
              <div class="result"
      ...
      <div class="product">
        ...
        <div class="enclosure" id="orderAudit">
          ...
          <el-row :gutter="80" v-if="approvalOrderInfo.fileList && approvalOrderInfo.fileList.length != 0">
            <el-col :span="8" v-for="file,index in approvalOrderInfo.fileList" :key="index">
          ...
          <div class="clearfix" v-if="approvalOrderInfo.approvalStatus == 0">
            ...
            <div class="fl">
              ...
              <textarea @focus="showErrorInfo=false;errorInfo='';" v-model="remark"
                        placeholder="此处填写您对订单的审批理由"></textarea>
              <p class="err-info" v-if="showErrorInfo">{{errorInfo}}</p>
    ...
    <section class="approval-record">
      <h4>审核记录</h4>
      <ul v-if="approvalOrderInfo.recordList">
        <li v-for="record,index in approvalOrderInfo.recordList" :key="index">
          <div :style="{background:colorArr[index%colorArr.length]}" class="header">
            {{record.userName.substring(0, 1)}}
          </div>
          <div class="admin clearfix">
            <p class="fl">{{record.userName}}<span class="color-gray">{{record.userPhone}}</span></p>
            <p class="fr">{{approvalOrderInfo.recordList.length - index}}# <span
              class="color-gray">{{record.approvalTime}}</span></p>
          </div>
          <p class="company-name color-gray">{{record.companyName}}</p>
          <p class="status" :class="{success: record.approvalResult == 1,fail: record.approvalResult == 0}">
            {{record.approvalResult == 1 ? '审批通过' : '审批拒绝'}}</p>
          <p class="reason">{{record.approvalRemark}}</p>
        </li>
      </ul>
    </section>
    <el-dialog title="审批操作" :visible.sync="approvalDialog" width="420px" class="custom">
      <p>确认审批{{approvalText}}吗?</p>
      <div slot="footer">
        <el-button @click="approvalDialog = false">取 消</el-button>
        <el-button type="primary" :loading="isLoading" @click="approval">审批{{approvalText}}</el-button>
      </div>
    </el-dialog>
  </div>
</template>
    ...
    methods: {
      ...
      submitApproval() {
        if (this.remark.trim() == '') {
          ...
          this.errorInfo = '备注不能为空!';
          return;
        }
        if (this.isPass == 1) {
          this.approvalText = '通过';
          this.approvalDialog = true;
        } else if (this.isPass == 2) {
          this.approvalText = '拒绝';
          this.approvalDialog = true;
        }
      },
      approval() {
