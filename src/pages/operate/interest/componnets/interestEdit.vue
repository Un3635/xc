    ...
    <d2-container>
        ...
        <div class="detail"
            ...
            <el-form :model="paramsData" :rules="rules" ref="paramsForm" label-width="150px" class="rule-form">
                ...
                <el-form-item label="排序："  v-if="tableData.name === '榜单' || tableData.name === '推荐'">
                    <span class="text">{{tableData.sort}}</span>
                </el-form-item>
                <el-form-item label="排序："  required v-else>
                    <el-input-number class="item-number" v-model="paramsData.sort" controls-position="right" :min="3" :max="127"></el-input-number>
                </el-form-item>
...
<script>
  export default {
    ...
    methods:{
      ...
      getTableData(){
        let vm = this;
        this.axiosGET(vm, vm.$API.API.interestDetail+vm.id,
     ...
     //成功回调
      ...
      setTableData(_data){
          ...
          this.paramsData.homeShow = _data.homeShow;
          obj.homeShow = _data.homeShow ? '显示' : '不显示';
          this.tableData = obj;
      },
      //设置表格数据
      submitForm(formName){
