...
<script>
  ...
  export default {
    ...
    watch: {
      '$route': {
        handler(val) {
          if(val.path === '/operate/tag' || val.path === '/operate/tag/'){
            this.getTableData();
          }
        }
      },
    ...
    methods: {
      ...
      getTableData(){
        let vm = this;
        this.axiosGET(vm, vm.$API.API.labelList,
      ...
      setTableData(_data){
        ...
        for (let i = 0; i <_data.length; i++) {
          ...
          switch(_data[i].partOfSpeech){ //词性
            case 1:
              obj.partOfSpeech = '中性';
              break;
            case 2:
      ...
      handleDetail(index, row) {
        this.$router.push({path: '/operate/tag/tagEdit', query: {id: row.id}})
      },
      //新增
      addOption(){
        this.$router.push({path: '/operate/tag/tagAdd'})
      }
    },
    mounted(){
      this.getTableData();
