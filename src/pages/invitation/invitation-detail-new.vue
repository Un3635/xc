            ...
            <div class="figure">
              ...
              <div class="figcaption">
                ...
                <h2 v-else class="calValue" :class="{'calValue2':rewardType===1}">
                  {{inviteStatus | cryptocurrency}}{{getUnit(rewardType)}}</h2>
                <div class="tobeDeliver">
                  <span v-if="rewardType ===4">冻结中 {{tobeDeliver}}元</span>
                  <span v-else-if="rewardType!==1">冻结中 {{tobeDeliver | cryptocurrency}}{{getUnit(rewardType)}}</span>
                  <span v-else>待交割 {{tobeDeliver}}{{getUnit(rewardType)}}</span>
                </div>
              </div>
              <div class="inviteData">
                <flexbox justify="space-between" class="flexbox">
                  ...
                  <flexbox-item v-if="rewardType===1 || rewardType===4">
                    <div class="itemTop">历史获得({{getUnit(rewardType)}})</div>
...
<script>
  ...
  export default {
    data() {
      return {
        ...
        inviteTipArr: [
          {
            title: '有效算力',
            inviteTip: ['您邀请的好友中，未在挖链APP中获取30WT的用户', '好友已达成奖励所需的要求']
          },
          {
            title: '可用BTC',
            inviteTip: ['您邀请的好友中，未在挖链APP中获取30WT的用户', '好友已达成奖励所需的要求']
          ...
          {
            title: '已返现金额',
            inviteTip: ['您邀请的好友中，未在挖链APP中获取30WT的用户', '']
          }
        ],
        inviteTip: '',
        productNum: '冻结数量(ETH)',
    ...
    methods: {
      ...
      initReWardType() {
        let unit;
        let {title, inviteTip} = this.inviteTipArr[Number(this.rewardType) - 1];
        this.title = title;
        this.inviteTip = inviteTip[this.tabelIndex];
        unit = Number(this.rewardType) === 1 ? this.computingUnit : this.currencyName;
        // 获取头部数据
        const getTopInfo = function(o, req, param1, param2) {
          req.then((res) => {
            o.tobeDeliver = res[param1] || 0;
            o.inviteStatus = res[param2] || 0;
          });
        };
        if (Number(this.rewardType) === 1) {
          // 算力
          getTopInfo(this, UserAPI.getcomputingIfor(this.computingUnit), 'freezenRewardCount', 'useRewardCount');
        } else if (Number(this.rewardType) === 4) {
          // 返现
          getTopInfo(this, UserAPI.balanceInfo(), 'alreadyCashBack', 'cashBackFreezeMoney');
        } else {
          // 其他算力
          getTopInfo(this, UserAPI.getCurrencyInfor(this.currencyId), 'rewardFreezeNumber', 'totalRewardNumber');
        }
        const getRewardInfo = function(o, req) {
          req.then((res) => {
            o.noRewardCount = res.noRewardCount || 0;
            o.alreadyRewardCount = res.alreadyRewardCount || 0;
            o.totalRewardCount = res.totalRewardCount || 0;
          });
        };
        if (Number(this.rewardType) === 4) {
          // 已返现金额
          getRewardInfo(this, UserAPI.getAwardOrderInfo(2));
        } else {
          // 其他
          getRewardInfo(this, UserAPI.getUserEntrustRewardInfo(this.rewardType, unit));
        }
      },
      ...
      inviteTabClick(index) {
        ...
        this.rewardStatus = index;
        this.inviteTip = this.inviteTipArr[Number(this.rewardType) - 1]['inviteTip'][index];
