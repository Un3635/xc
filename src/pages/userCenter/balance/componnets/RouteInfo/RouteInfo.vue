...
<script>
  ...
  export default {
    ...
    methods: {
       ...
       getTableData(){
          let vm = this;
          this.axiosGET(vm, vm.$API.API.balanceList,
       ...
       setTableData(_data){
          ...
          for (let i = 0; i <_data.length; i++) {
            let obj = {};
            obj.mid = _data[i].mid;
            obj.balance = _data[i].balance;
            obj.withdrawCash = _data[i].withdrawCash ? _data[i].withdrawCash : '-';
            myData[i] = obj;
          }
       ...
       handleDetail(index, row) {
        this.$router.push({path: '/userCenter/balance/balanceDetail', query: {mid: row.mid}})
       },
    },
    mounted(){
      this.getTableData();
