<template>
  <div class="d2-layout-header-aside-menu-side">
    <el-menu
      ...
      :unique-opened="true"
      :default-active="this.active.slice(0, this.active.length-1)"
      ref="menu"
...
export default {
  ...
  watch: {
    ...
    '$route.matched': {
      handler (val) {
        let routeList = ['/order/recharge', '/order/withdraw/', '/user/', '/assets/']
        this.active = val[val.length - 1].path
        /*this.$nextTick(() => {
          if (this.menuAside.length > 0) {
            for (let i = 0; i < routeList.length; i++) {
              if (this.active.indexOf(routeList[i]) > -1) {
                  this.$refs.menu.activeIndex = routeList[i]
              }
            }
          }
        })*/
