 ...
 * @param {string}        params.channel  1:线下充值  2:线下付款
 ...
 * @param {string}    formData.file  图片
 * @return {Promise<*>}
 */
const uploadImg = params => api.$post(AppConfig.uploadUrl, params, {
 ...
 * @param {string} returnURl 支付成功返回url
 * @param {string} type 设备类型，PC -电脑网页，MOBILE - 手机网页
 * @return {Promise<*>} 正常结果为resolve,异常结果reject
 */
const rechargeAlipay = ({orderNo, returnURl, type}) => TokenAPI.getValid()
 ...
 * @return {Promise<*>}
 */
const alipayResult = orderNo => TokenAPI.getValid()
 ...
 * @param {string}  token
 * @param {string}  orderNo  支付订单号
 * @param {string}  openId  微信openId（只在微信浏览器里传）
 * @param {string}  type  设备类型，PC -电脑网页，MOBILE - 手机网页，WECHAT-微信客户端
 * @return {Promise<*>}
 */
const weChatPay = ({orderNo, openId, type}) => TokenAPI.getValid()
  .then(token => api.$post(AppConfig.weChatPay, {
    token,
    orderNo,
    openId,
    type
