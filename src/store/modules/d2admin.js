...
export default {
  state: {
    ...
    themeList,
    // 现在激活的主题
    themeActiveName: themeList[4].name, // 这应该是一个名字 不是对象
    // 可以在多页 tab 模式下显示的页面
    pagePool: [],
    // 当前显示的多页面列表
    pageOpenedList: [
  ...
  actions: {
    startLogin({state, commit, rootState}, {vm}){
      commit('getCodeId', vm)
    },
    /**
     * 登陆
     ...
     */
    d2adminLogin ({state, commit, rootState}, {vm, userName, password, captcha, checkId, token}) {
      let loginData ={
        userName,
        password,
        captcha,
      ...
      vm.$axios({
        ...
        url: vm.$API.URL + vm.$API.API.login,
        params: loginData
      })
        .then((res) => {
          if(res.code === '6000'){
            vm.$message.error('验证码错误');
            //注释——验证码错误时不刷新验证码, 验证码超时(1200s)刷新
            let nowTime = new Date().getTime();
            if((nowTime - state.captchaTime) >= 1200000){
            ...
            state.userAuthority = res.data;
            ////////////////设置菜单数据开始//////////////////
            state.menuData = res.data.modulePermissions;
            state.authorityData = res.data.permissions;
            // console.log(state.menuData)
            state.menuAside = [];
            for(let item in state.menuData){
              if(state.menuData[item].isMenu){
                let tempMenu = {
                  title: '',
                  name: '',
                  icon: '',
                ...
                tempMenu.icon = state.menuData[item].menuIcon;
                tempMenu.title = state.menuData[item].showName;
                tempMenu.name = state.menuData[item].showName;
                for(let i=0; i<state.menuData[item].modulePermissionBoList.length; i++){
                    let tempchild = {
                      title:'',
                      name:'',
                      path:'',
                    ...
                    tempchild.title = state.menuData[item].modulePermissionBoList[i].showName;
                    tempchild.name = state.menuData[item].modulePermissionBoList[i].showName;
                    tempchild.path = state.menuData[item].modulePermissionBoList[i].pageUrl;
                    tempchild.permissionName = state.menuData[item].modulePermissionBoList[i].permissionName;
                    if(state.authorityData && state.authorityData.length >0){ //根据权限设置编辑、新增按钮状态（是否禁用）
                      for(let j=0;j<state.authorityData.length;j++){
                        if(Number(res.data.isSuper) === 1 || (state.authorityData && state.authorityData.indexOf( tempchild.permissionName) !== -1)){
                          tempchild.btnState = true;
                        } else {
                          tempchild.btnState = false;
                        }
                      }
                    } else {
                    ...
                    tempMenu.children.push(tempchild);
                }
                state.menuAside.push(tempMenu)
              }
            };
            // console.log(state.menuAside);
            if(state.menuData !== ''){
              util.cookies.set('d2adminMenuAside', state.menuAside); //把菜单数据保存到cookie
              // console.log(util.cookies.get('d2adminMenuAside'))
            }
            commit('d2adminMenuAsideSet', JSON.parse(util.cookies.get('d2adminMenuAside')));
            ////////////////设置菜单数据结束/////////////////
            ...
            util.cookies.set('userInfoId', res.data.id); //保存用户id
            // 设置 vuex 用户信息
            commit('d2adminUserInfoSet', {
              name: res.data.userName
            });
            util.cookies.set('userAuthorityAll',res.data.permissions);
            util.cookies.set('isSuper',res.data.isSuper);
            if(res.data.status === 3) {
              commit('setShowPwdPop',true);
              util.cookies.set('pwdPopCloseBtnDisabled',true);
            } else{
    ...
    d2adminLogout ({ state, commit, rootState }, { vm, confirm }) {
      ...
      function logout () {
        ...
        util.cookies.remove('userInfoId');//删除用户id
        util.cookies.remove('userAuthorityAll'); //删除用户权限
        util.cookies.remove('isSuper'); //删除用户权限
        util.cookies.remove('pwdPopCloseBtn'); //删除修改密码状态
        util.cookies.remove('pwdPopCloseBtnDisabled');  //删除密码弹窗按钮禁用
        // 跳转路由
        vm.$router.push({
          name: 'login'
        })
  ...
  mutations: {
    ...
    getCodeId(state,vm){
      vm.$axios({
        method: 'get',
        url: vm.$API.URL + '/kaptcha/captcha/getCaptchaId',
        data: {}
      }).then((res) => {
        state.captchaId = res.captchaId;
        state.codeImgUrl = vm.$API.URL + '/kaptcha/captcha.jpg?captchaId=' + res.captchaId;
        state.captchaTime = new Date().getTime();
      }).catch((err) => {
        console.log(err)
      })
    },
    ...
    *  修改密码弹窗状态修改
     */
    setShowPwdPop(state,param){
      state.showPwdPop = param;
    },
    /**
     *  图片弹窗状态修改
     */
    setShowPicPopOn(state,param){
      state.showPicPop = true;
      state.showPicParam = param;
    },
    setShowPicPopOff(state){
      state.showPicPop = false;
      state.showPicParam = {type:'',content:''};
    },
    /**
     *  获取按钮权限
     */
    getAuthorityBtnState(state,params){
      let authority = JSON.parse(util.cookies.get('d2adminMenuAside'));
      for(let i=0;i<authority.length;i++){
        for(let j=0;j<authority[i].children.length;j++){
          if(params === authority[i].children[j].path ){
            state.authorityBtnState = authority[i].children[j].btnState;
            return false;
          }
        }
      }
    },
    /** 改变刷新按钮状态（礼物管理页-排序弹窗使用)**/
    changeRefreshState(state,params){
    ...
    d2adminLoginSuccessLoad (state) {
      ...
      this.commit('d2adminPageOpenedListLoad')  //Julia - 关闭
      // DB -> store 数据库加载这个用户之前设置的侧边栏折叠状态
      this.commit('d2adminMenuAsideCollapseLoad') //Julia - 关闭
    },
    /**
     * @description 设置用户名
     * @class userInfo
    ...
    d2adminThemeSet (state, themeName) {
      ...
      if (theme) {
        ...
        // 设置为列表第一个主题
        state.themeActiveName = state.themeList[4].name // 设置第四个主题为默认主题
      }
      // 将 vuex 中的主题应用到 dom
    ...
    d2adminThemeLoad (state) {
      this.commit('d2adminUtilDb2VuexByUuid', {
        key: 'themeActiveName',
        defaultValue: state.themeList[4].name
      })
      this.commit('d2adminTheme2dom')
    },
    /**
