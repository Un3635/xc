  ...
  <div>
    ...
    <div class="wl_upload_card" ref="card" v-loading="loading" v-show="type.indexOf('img')>=0||!loading">
      <div class="wl_upload_card_before">
        <el-upload :action="action" :headers="uploadheader" :multiple="false" :limit="1000" :name="'logo'" :show-file-list="false" :on-error='on_error' :on-success='on_success' :before-upload="on_before" :on-progress='on_progress' :http-request="http_request" ref="upload">
    ...
    <div class="wl_upload_progress" v-show="!(type.indexOf('img')>=0||!loading)">
      <div class="wl_upload_progress_inner">
        <div class="wl_upload_progress_value" ref="value">{{percent|filepercent}}%</div>
        <div class="wl_upload_progress_line" ref="line"></div>
        <div class="wl_upload_progress_info">
          当前上传速度：{{progress_speed|filespeed}}MB/s 预计剩余时间：{{progress_lefttime|filelefttime}}秒 已上传：
          {{(filesize*percent/100)|filesize}}MB
          /
          {{filesize|filesize}}MB
          <span @click="start" v-if="instop">继续上传</span>
          <span @click="stop" v-if="!instop">暂停上传</span>
...
<script>
  ...
  import upload_img from "./upload_img.js";
  import {
    checkFileUploadInfo,
    webUserVideofileUploadCancel
  ...
  export default {
    ...
    data() {
      return {
        ...
        OSSFileName: "",
        partFileNums: [],
        gXhr: null, //全局xhr对象用于暂停取消请求
        orignalName: "", //文件原始名称
        originalFileSize: 0, //文件原始大小
        instop: false, //暂停状态
        option: null, //mp4mp3上传文件配置用,
        erruploading: ""
      };
    },
    mounted() {
      if (this.type == "img") {
        ...
        this.action = AppConfig.apiEndpoint + "/webUserVideo/uploadPartFile.wl";
      }
    },
    methods: {
      message({
      ...
      on_before(file) {
        ...
        if (this.type == "mp4" || this.type == "mp3") {
          return checkFileUploadInfo({
            ...
            var { partFileNums, OSSFileName, error: erruploading } = res;
            this.erruploading = erruploading;
      ...
      on_progress(event, file, fileList) {
        if (this.type == "mp4" || this.type == "mp3") {
          if (event.OSSFileName) {
            this.OSSFileName = event.OSSFileName;
          }
      ...
      start() {
        ...
        checkFileUploadInfo({
          ...
          this.erruploading = erruploading;
          if (this.erruploading == "uploading") {
            this.loading = false;
            this.message({
              title: "文件上传中,请稍后再试",
              type: "warning"
            ...
            this.gXhr=upload(this.option, this.OSSFileName, this.partFileNums);
          }
        });
      },
      resetprogress_data() {
      ...
      http_request(option) {
        ...
        if (this.type.indexOf("img") >= 0) {
          ...
          this.orignalName = option.file.name;;
           this.originalFileSize = option.file.size;
          if (this.erruploading == "uploading") {
            this.message({
              title: "文件上传中,请稍后再试",
              type: "warning"
            ...
            this.gXhr=upload(option, this.OSSFileName, this.partFileNums);
          }
        }
      }
    },
