      ...
      <div class="page-content__inner">
        <table class="tb-pool">
          ...
          <tr v-for="item in allPollStatus" :key="item.coin">
            ...
            <td>
              <router-link :to="'/pool/'+item.coin">{{item.coin + '.pool.hc.top'+':'+ (port[item.coin]||'')}}
              </router-link>
            </td>
            <td>{{item.poolhash}}</td>
            ...
            <td @click="goPollBlock(item.coin)">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      serchWallet(walletAddress) {
        if (!walletAddress.trim()) {
          this.serchTip = '请填写正确的钱包地址';
          return;
        }
        poolAPI.searchWallet(walletAddress).then(res => {
