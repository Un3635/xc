<template>
  <div class="miner-detail">
    <div class="miner-breadcrumb">
      <div class="miner-breadcrumb-detail">
        <app-breadcrumbs/>
      </div>
    </div>
            ...
            <div class="detail-content">
                      ...
                      <el-popover v-if="act.activityType===2&&rewardList" placement="bottom" width="400"
                        ...
                        <table class="tb-rule">
                          ...
                          <tr v-for="(rule,idx) in rewardList" :key="idx">
                            ...
                            <td>{{rule.inviterWithdrawal}}元/台</td>
                            <td>{{rule.inviteeDiscounts}}元/台</td>
                  ...
                  <div class="shop-item shop-item-price">
                    ...
                    <dd>
                      <span class="currentPrice" v-if="isShowSalePrice">
                        <span v-if="goodsInfo.status===1">{{goodsInfo.goodSalePrice | currency}}元/台</span>
                        <span v-else-if="goodsInfo.status===2">{{goodsInfo.goodSalePrice}}</span>
                        <span v-else>{{goodsInfo.goodSalePrice}}元/台</span>
                      </span>
                      <span class="originPrice" v-if="goodsInfo.status!==2&&goodsInfo.goodsPrice">
                        <span>原价：</span>
                        <span v-if="goodsInfo.goodsPrice">{{goodsInfo.goodsPrice | currency}}元/台</span>
                      </span>
                    </dd>
                  </div>
                </div>
              ...
              <div class="deliver">
                <div class="deliver-item">
                  <dt>发货时间</dt>
                  <dd>{{goodsInfo.startSendTime | date('MM月DD日')}}-{{goodsInfo.endSendTime | date('MM月DD日')}}</dd>
                </div>
                <div class="deliver-item num">
                  ...
                  <dd>
                    ...
                    <span class="limit">台</span>
                    <span class="limit-num">
                      <span v-if="goodsInfo.buyLimit>0">{{goodsInfo.buyLimit}}台起购</span>
                      <span v-if="goodsInfo.buyLimit>0&&goodsInfo.orderLimit>0">,</span>
                      <span v-if="goodsInfo.orderLimit>0">每笔限购{{goodsInfo.orderLimit}}台</span>
                      <span v-if="(goodsInfo.buyLimit>0||goodsInfo.orderLimit>0)&&goodsInfo.userLimit>0">,</span>
                      <span v-if="goodsInfo.userLimit>0">每人限购{{goodsInfo.userLimit}}台</span>
                    </span>
                  </dd>
                </div>
          ...
          <div class="tabs">
            <div class="tabs-header">
              <ul class="clearfix">
                ...
                <li @click="robotSelectIndex=1" :class="{'active':robotSelectIndex===1}">矿机参数</li>
                <li @click="robotSelectIndex=2" :class="{'active':robotSelectIndex===2}">售后政策</li>
              </ul>
            </div>
            <div class="tabs-body">
...
<script>
  ...
  import {UPDATE_PRE_ORDER} from '../../store/mutation-types';
  ...
  export default {
    ...
    breadcrumb() {
      return {
        label: this.goodsInfo.goodsName || '矿机详情',
        parentsList: [
          {
            path: '/product',
            label: '矿机'
    ...
    methods: {
      ...
      submitOrder() {
        ...
        this.count = +this.count;
        if (this.count > max) {
          return this.$alert(`该矿机单人限购${max}台`);
        }
      ...
      initPage() {
        const batchGoodsId = this.$route.query.pId;
        ProductAPI.getGoodsInfo(batchGoodsId)
          .then(res => {
