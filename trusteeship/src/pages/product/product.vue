            ...
            <div class="product-count clearfix">
              <div class="left">
                ...
                <p class="original-cost">
                  <span v-if="item.goodsPrice" class="decoration">原价：{{item.goodsPrice | currency}}元</span>
                  <span v-else>矿机售价</span>
                </p>
              </div>
              <el-button class="right" type="primary" :class="{'is-disabled':item.status!==0&&item.status!==1}"
                         @click="$router.push('/product/detail?pId='+item.goodsId)">
                {{btnText(item)}}
              </el-button>
            </div>
          </div>
      ...
      loadProductCategory() {
        ProductAPI.getGoodsCategoryList()
          ...
          .then(res => {
            ...
            this.categoryId = this.$route.params.categoryId || (res[0] || {}).id;
          })
          .catch(() => {
          });
      },
      getGoodsList() {
        ProductAPI.getProductsList(this.categoryId, this.pageNum, this.pageSize)
          .then(res => res.list || [])
          .then(res => {
            res.forEach(item => {
