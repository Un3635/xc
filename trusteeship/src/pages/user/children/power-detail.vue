        ...
        <div>
          ...
          <table class="table-end">
            ...
            <tr v-for="item in tableData" :key="item.orderNo">
              <td><span class="data-num"
                        @click="$router.replace({path:'/i/power/power-info',query:{powerId: item.orderNo,currencyName:symbol}, append:true})">{{item.orderNo}} </span>
              </td>
            ...
            <tr v-for="item in tableData" :key="item.orderNo" :class="{'color-delivery-wait':item.status === 0}">
              <td>
                ...
                <span :class="{'data-num':item.status > 0}"
                      v-if="item.status > 0"
                      @click="$router.push({path:'/i/power/power-info',query:{powerId: item.orderNo,currencyName:symbol}, append:true})">
                    {{item.orderNo}}
                  </span>
              </td>
              <td>{{item.goodsCount || 0}}</td>
