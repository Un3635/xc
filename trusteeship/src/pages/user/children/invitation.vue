      ...
      <div class="bottom-container">
        ...
        <div class="types">
          ...
          <div class="type__desc" v-if="selected===4">
            ...
            <p class="message" v-if="inner_selected===0">您邀请的好友，还未支付矿机订单的用户</p>
        ...
        <div class="table-wrapper">
          ...
          <el-table class="tb" :data="userData" border stripe>
            ...
            <el-table-column label="未奖励数量（ETH）" v-if="selected===3&&inner_selected===0">
              <template slot-scope="scope"><span>{{scope.row.rewardCount || 0}}台</span></template>
            </el-table-column>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      loadData(page) {
        if (this.selected === 0) {
          return UserAPI.allInviteeInfo(0, page);
        }
        const status = this.inner_selected;
        if (this.selected === 4) {
          return UserAPI.awardList(status, page)
            .then(res => res || {})
            .then(res => {
        ...
        return UserAPI.rewardInviteeInfo(type, status, page);
      },
