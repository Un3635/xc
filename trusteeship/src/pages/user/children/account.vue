    ...
    <div class="page-content">
      <section class="c-section">
        <h4 class="c-section__title">
          账户余额
          <router-link class="c-text-btn" :to="{path:'account/account-balance'}">
            查看明细 &gt;
          </router-link>
        </h4>
      ...
      <section class="c-section" v-for="item in currencyList" :key="item.currencyId">
        <h4 class="c-section__title">
          ...
          <router-link class="c-text-btn"
                       :to="{path:'account-detail',query:{currencyId:item.currencyId},append:true}">
            查看明细 &gt;
          </router-link>
        </h4>
        <div class="c-section__content clearfix">
          <div class="left">
            总额<strong>{{(item.currencyCount + item.currencyFreezeNumber)| cryptocurrency}}</strong>{{item.currencyName}}
          </div>
          <div class="left">
            可用<strong>{{item.currencyCount| cryptocurrency}}</strong>{{item.currencyName}}
          </div>
          <div class="left">
            冻结<strong>{{item.currencyFreezeNumber| cryptocurrency}}</strong>{{item.currencyName}}
          </div>
          <div class="right text-right">
