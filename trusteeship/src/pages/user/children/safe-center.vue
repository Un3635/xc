<template>
  <div class="page">
      ...
      <div class="c-tile-list">
        <div class="c-tile-list__item">
          ...
          <div class="item-inner account">
            <h3>我的账户</h3>
            <p>
              <span>绑定手机: {{info.moblie?hidePhone(info.moblie, '3', '7'):'未绑定'}}</span>
              <el-button v-if="!info.moblie" class="right" size="mini" @click="bindPhoneDialog = true">去绑定</el-button>
            </p>
            <p>
              <span>绑定邮箱: {{info.mailboxAccount?hideEmail(info.mailboxAccount, '3'):'未绑定'}}</span>
    ...
    <el-dialog class="custom" :title="info.hasPassword ? '修改密码' : '设置密码'" :visible.sync="changePasswordDialog"
    ...
    <el-dialog class="custom" title="绑定邮箱" :visible.sync="bindEmailDialog" width="420px"
               @close="errors.clear('emailBindForm')">
    ...
    <el-dialog class="custom" title="绑定手机" :visible.sync="bindPhoneDialog" width="420px"
               @close="errors.clear('phoneBindForm')">
    ...
    <el-dialog class="custom" :visible.sync="realNameAuthDialog" width="420px"
               @close="errors.clear('realNameForm')">
    ...
    <el-dialog class="custom" :visible.sync="bindBankcardDialog" width="420px"
               @close="errors.clear('bankcardForm')">
      <p class="dialog-title" slot="title">
        <strong>添加银行卡</strong>
    ...
    <el-dialog class="custom" :visible.sync="safeVertifyDialog" title="安全验证" width="420px">
...
<script>
  ...
  export default {
    computed: {
      ...
      //      newAddress(){
      //          return this.istodayAdd || this.currentOperatingItem.address.length>5
      //      }
    },
    data() {
      return {
    ...
    methods: {
      ...
      getUserInfo() {
        UserApi.userInfo()
          ...
          .then(res => {
            ...
            let currencyInfos = JSON.parse(res.address || '');
            currencyInfos.sort((a, b) => a.currencyId - b.currencyId);
            this.currencyList = currencyInfos;
      ...
      getModeList(res) {
        ...
        if (res.moblie) {
          this.modeList.push({
            type: '手机验证',
            label: '手机验证   ' + (res.countryNum ? res.countryNum + ' ' : '') + this.hidePhone(res.moblie, 3, 7),
            value: 1
          });
          // this.mode = 1;
        }
        if (res.mailboxAccount) {
          this.modeList.push({
            type: '邮箱验证',
            label: '邮箱验证   ' + this.hideEmail(res.mailboxAccount, 3),
            value: 0
          });
          // this.mode = 0;
        }
      ...
      sendCode(msg, type, num) {
        ...
        if (msg === 'email') {
          ...
          if (this.verifyMethod === 1) {
            ...
            params.account = this.info.mobile;
          } else {
            params.countryNum = '';
