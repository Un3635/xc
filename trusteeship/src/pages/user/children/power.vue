    ...
    <div class="clearfix box-detail">
      <div class="left detail-left">
        ...
        <span v-if="selectedItem.id"
              @click="$router.push({path:'power-detail',query:{currencyId:selectedItem.id}, append:true})">算力明细 ></span>
      </div>
        ...
        <table>
          ...
          <tr v-for="(item,idx) in tableData" :key="item.orderNo">
            <td class="col-id" :style="{height:rowHeight(idx+1)}">
              <span class="data-num"
                    @click="$router.push({path:'power-info',query:{powerId: item.orderNo,currencyName:selectedItem.name}, append:true})">{{item.orderNo}} </span>
            </td>
          </tr>
          </tbody>
        </table>
