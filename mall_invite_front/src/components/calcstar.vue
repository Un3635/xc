<template>
  <div>
    <div class="clacstar-them"></div>
    <div class="clacsrtar-content _content">
      <!-- <div class="_wrap"> -->
        <div class="calc-title-list" ref="titleListWrap">
          <ul class="_wrap_ul" ref="titleGroup">
            <li class="calc-title" :class="{active: index === titleIdx}" v-for="(item, index) in titleList" :key="index" @click="changeTitle(item.id, index)">{{item.name}}</li>
          </ul>
      </div>
      <!-- </div> -->
      
      <div class="calc-detail-list">
        <div class="zan-wu-data" v-show="!calcList || !calcList.length">暂无收益</div>
        <ul>
          ...
          <!-- <li v-for="item in 100" :key="item" @click.stop="checkDetail(1)">{{item}}</li> -->
        </ul>
      </div>
      <div class="pagination-wrap" v-show="calcList && calcList.length">
        <el-pagination
          background
          :page-size="12"
...
export default {
  ...
  methods: {
    ...
    getGoodsList() {
      let param = {'page_num': this.pageNum, 'page_size': this.pageSize};
      if(this.powerTypeId) {
...
.clacstar-them {
  ...
  min-width: 1200px;
  height: 512px;
  background: url("../../static/image/yunsanli_bg.png") center no-repeat;
  }
.clacsrtar-content{
  margin-top: 32px;
  ...
  .calc-detail-list {
    ...
    .calc-detail {
      ...
      .btn {
        ...
        text-align: center;
        // border:1px outset #8557FF;
          border:1px solid transparent;
        // border-image: -webkit-linear-gradient(#8557FF 0%, #4D2EEA 95%);
        // border-image: -moz-linear-gradient(#8557FF 0%, #4D2EEA 95%);
        // border-image: linear-gradient(#8557FF 0%, #4D2EEA 95%);
        // background-image: linear-gradient(#8557FF 0%, #4D2EEA 95%);
