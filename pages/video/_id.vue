    ...
    <div class="page-news-inner clearfix">
      ...
      <div class="page-news-left">
        ...
        <div class="page-news-info">
          ...
          <span class="page-news-info-num">
            {{data.totalHotNum||0}}</span>
        </div>
        <div id="youkuplayer" style="width:100%;height: 433px;" v-loading="!isPlayerLoaded"></div>
        <div class="page-news-content article" v-html="data.content"> </div>
        <div class="page-news-tags">
                    ...
                    <div class="page-news-comment-hot-item-hd-ctrl">
                      ...
                      <el-dropdown class="page-news-comment-hot-item-hd-ctrl-report" @command="report(i)">
                        ...
                        <el-dropdown-menu slot="dropdown" class="page-news-comment-hot-item-hd-ctrl-report-items">
                          ...
                          <template v-else>
                            <el-dropdown-item command="1" v-if="i.userId!=$store.state.token.split('_')[0]">举报</el-dropdown-item>
                        ...
                        <div class="page-news-comment-hot-item-ft-item-hd-ctrl">
                          ...
                          <el-dropdown class="page-news-comment-hot-item-ft-item-hd-ctrl-report" @command="report(ii)">
                            ...
                            <el-dropdown-menu slot="dropdown" class="page-news-comment-hot-item-ft-item-hd-ctrl-report-items">
                              ...
                              <template v-else>
                                <el-dropdown-item command="1" v-if="ii.userId!=$store.state.token.split('_')[0]">举报</el-dropdown-item>
                                <el-dropdown-item command="2" v-else>删除</el-dropdown-item>
                              </template>
                            </el-dropdown-menu>
                          </el-dropdown>
      ...
      <div class="page-news-right">
        <div class="page-news-author">
          ...
          <div class="page-news-authorviewinfo clearfix" style="border-bottom:0px;">
            <div class="page-news-authorviewinfoitem" @click="goauthorlist">
              <div class="v">{{data.followCount}}</div>
              <div class="k">订阅量</div>
            </div>
            <div class="page-news-authorviewinfoitem" @click="goauthorlist">
              <div class="v">{{data.videoTotalNum}}</div>
              <div class="k">文章总数</div>
            </div>
          </div>
          <!-- <div class="page-news-author-recentArticles" v-if="list.length>0">
            ...
            <div class="page-news-author-recentArticles-list">
              <div class="page-news-author-recentArticles-item" v-for="(i,idx) in list" :key="idx">
                <a :href="`/video/${i.mediaId}.html`" class="page-news-author-recentArticles-item-title" :title="i.title" target="_blank">{{i.title}}</a>
...
<script>
  ...
  export default {
    components: { hotNews, AdvFull, ThumbnailAds, cArticleList, cArticleShare },
    data() {
      return {
        advlist: [],
        follow: 0,
    ...
    methods: {
      ...
      oncollect() {
        ...
        collectupdateCollect({
          uniteId: this.$route.params.id.split(".html")[0],collectType:2
        })
          .then(res => {
            if (res.status == 1) {
            ...
            this.message({ title: res.msg });
          })
          .catch(err => {
            this.message({ title: err.msg, type: "warning" });
          });
      },
      login() {
      ...
      goauthorlist() {
        var url = `/author/${this.data.clientType}/${this.data.userId}`;
        window.open(url);
      },
      getfollowstate(data) {
        if (data == 1) {
          var authorType = this.data.clientType;
          var authorUserId = this.data.userId;
          var mediaType = 2;
      ...
      cmt() {
        addComment({
          ...
          authorUserId: this.data.userId,
          content: this.cmt_content,sourceType : 3,
        })
          .then(res => {
            this.cmt_content = "";
            res.userReplyList = [];
    ...
    async asyncData(ctx) {
      return getVideoById(ctx.route.params.id.split(".html")[0])
        .then(res => res || {})
        .then(data => {
    ...
    head() {
      return {
        ...
        meta: [
          ...
          {
            property: "og:url",
            content: "https://www.walian.cn/video/" + this.data.id + ".html"
          },
          { property: "og:site_name", content: "挖链网" },
          {
            property: "article:published_time",
            content: new Date(this.data.pubTime)
          },
          { property: "article:author", content: this.data.author },
          {
            property: "og:published_first",
            content:
              "挖链网,https://www.walian.cn/video/" + this.data.id + ".html"
          }
        ]
      };
    }
