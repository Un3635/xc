<template>
  <div class="bind-authenticator">
    <h2 class="header">Google認証システム</h2>
    <div class="instructions">
      ...
      <ul>
        <li>
          <header>
            <i class="number">1</i>
            <p>Google認証システムをインストールする（GoogleAuthenticator）。</p>
          </header>
          <div class="clearfix content">
            ...
            <div class="left use-info">
              <div class="clearfix">
                <div class="store-icon left" @click="iosDialogVisible = true">
                  <i class="icon-apple">
                ...
                <el-button type="text" class="left" @click="goGuide('ios')">取扱説明
                </el-button>
              </div>
              <div class="clearfix">
                <div class="store-icon left" @click="androidDialogVisible = true">
                  <i class="icon-google">
                ...
                <el-button type="text" class="left" @click="goGuide('android')">取扱説明
                </el-button>
              </div>
            </div>
          </div>
        ...
        <li>
          <header>
            <i class="number">2</i>
            <p>Google認証システムで下のＱＲコードをスキャンしてください。</p>
          </header>
          <div class="content auth-qrcode">
            ...
            <div class="reload" v-else>
              <div>
                ...
                <p class="fail">デュアル・コードは失敗をロードする。</p>
                <button @click="initPage">再ローディング</button>
              </div>
            </div>
          </div>
        </li>
        <li>
          <header>
            <i class="number">3</i>
            <p>下のシークレットキーを安全な所に保存してください。</p>
          </header>
          <div class="content">
            <div class="secret-key">
              <span class="key">&nbsp;{{secretKey}}&nbsp;</span>
              <el-button type="text" v-clipboard:copy="secretKey" v-clipboard:success="onCopy"
                         :disabled="this.secretKey ==''">コピー
              </el-button>
              <el-button type="text" @click="save" :disabled="this.secretKey ==''">携帯に保存する</el-button>
            </div>
            <p class="tip">二段階認証を設定する前に、16桁のシークレットキーのコピーを書き留めるか保存し、安全な場所に保管してください。携帯電話を紛失、盗難、または消去した場合は、アカウントに戻るためにこのキーが必要になります</p>
          </div>
        </li>
        <li>
          <header>
            <i class="number">4</i>
            <p>下の枠にGoogle認証システムの６桁数字認証コードを入力してください</p>
          </header>
    ...
    <el-dialog
      title="iOS Google認証システム"
      :visible.sync="iosDialogVisible"
      width="420px">
      <div class="qrcode-box">
        <strong>ＱＲコードをスキャン</strong>
        <img src="../../../../assets/images/google-ios.png" alt="">
        <p>スキャンできない方はこちら</p>
        <a href="https://itunes.apple.com/jp/app/google-authenticator/id388497605?mt=8" target="_blank">
          インストールパッケージをダウンロード
        </a>
      </div>
    </el-dialog>
    <el-dialog
      title="Android Google認証システム"
      :visible.sync="androidDialogVisible"
      width="420px">
      <div class="qrcode-box">
        <strong>ＱＲコードをスキャン</strong>
        <img src="../../../../assets/images/google-android.png" alt="">
        <p>スキャンできない方はこちら</p>
        <a href="https://play.google.com/store/apps/details?id=com.google.android.apps.authenticator2&hl=ja"
           target="_blank">
          インストールパッケージをダウンロード
        </a>
      </div>
...
<script>
  ...
  export default {
    head() {
      return {
        title: 'Google認証_FinTo-ワンストップのマイニング機械委託、販売、クラウドマイニング賃貸のプラットフォーム'
      };
    },
    breadcrumb() {
      return {
    ...
    methods: {
      initPage() {
        UserAPI.googleAuth()
          ...
          .then(res => {
            ...
            return QRCode.toDataURL(res.qrBarcode);
          })
      ...
      goGuide(platform) {
        let url = window.location.origin + this.$router.resolve('guide-' + platform, null, true).href;
        window.open(url);
      }
    }
  };
