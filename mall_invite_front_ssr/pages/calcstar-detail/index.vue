<template>
  <div class="calc-detail-wrap">
    <div class="title-top"><div><span style="cursor:pointer;" @click="$router.push('/calcstar')">云算力</span> &gt; {{data.goodsName}}</div></div>
          ...
          <div class="calc-detail_content">
            <div class="con_innner">
              ...
              <div class="d_title_Idx1" v-show="d_title_Idx === 2">
                <h1 class="title">{{data.goodsName}}</h1>
                <ul>
...
<script>
  export default {
    ...
    mounted() {
      getajaxdata(API.goodsDetail, 'POST', {goods_id: this.$route.query.goodsId,initTimes: new Date().getTime()}, 'json', (res) => {
        this.data = res.data;
      })
    },
    methods: {
