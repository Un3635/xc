  ...
  <div class="profile">
    <el-tabs v-model="activeName">
      ...
      <el-tab-pane label="提币地址" name="getMoney">
        <div class="profile-info safe-center mB130">
          <el-form ref="form2" :model="formInfo" label-width="110px">
...
<script>
  ...
  export default {
    ...
    mounted() {
      // 获取钱包地址
      this.activeName = this.$route.query.active || 'infoCenter';
      this.getWalletAddressList();
      if (localStorage.getItem('userInfo') !== null ) {
        if( localStorage.getItem('userInfo') !== 'undefined' ) {
          this.userInfo = JSON.parse(localStorage.getItem('userInfo'));
          this.formInfo.name = this.userInfo.userName || phoneInit(this.userInfo.phone);
    ...
    methods: {
      ...
      getWalletAddressList() {
        // 获取提币地址
        getajaxdata(API.extractWalletAddressList, 'POST', {initTimes: new Date().getTime()}, 'json', (res) => {
      ...
      changeInfo() {
        ...
        getajaxdata(API.setData, 'POST', {
          ...
          if (res.code == 1000) {
            vm.isModal = true;
            vm.userInfo.userName = vm.formInfo.name;
            setDataToLocal('userInfo', JSON.stringify(vm.userInfo));
            vm.$store.commit('userNameStore', vm.formInfo.name);
            setTimeout(function () {
              vm.isModal = false;
            }, 3000);
          }else {
