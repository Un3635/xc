    ...
    <div class="item-detail">
      <div class="detail-table">
        <el-table :data="detailData" style="width: 100%" class="detail common-table"
          ...
          <el-table-column prop="actualOutputCoin" label="实际币量"></el-table-column>
            ...
            if(res.data.revenueDetailsVoList) {
              for (let i = 0; i < vm.detailData.length; i++) {
                if (vm.detailData[i].type == 2) {
                  ...
                  vm.detailData[i].actualOutputCoin = res.data.revenueDetailsVoList[i].actualOutputCoin + vm.detailData[i].coinAbbreviation.toUpperCase();;
                  vm.detailData[i].outputCoin = res.data.revenueDetailsVoList[i].outputCoin + vm.detailData[i].coinAbbreviation.toUpperCase();;
                }
                if (vm.detailData[i].electricityFeeType == 2 ){
                  vm.detailData[i].electricityFee = vm.detailData[i].electricityFee + '元'
                } else {
                  vm.detailData[i].electricityFee = vm.detailData[i].electricityFee + vm.detailData[i].coinAbbreviation.toUpperCase();
                }
                if (vm.detailData[i].serviceFeeType == 2 ){
                  vm.detailData[i].serviceFee = vm.detailData[i].serviceFee + '元'
                } else {
                  vm.detailData[i].serviceFee = vm.detailData[i].serviceFee + vm.detailData[i].coinAbbreviation.toUpperCase();
                }
              }
            }
          }
